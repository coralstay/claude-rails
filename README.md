...
에이전트(Claude Code)를 쓰기 위한 개인용 훅 모음집이다. Claude Code와 함께 작업할 때,
Claude가 내가 의도한 대로 행동하도록 만들기 위해 만들었다.
...
필요한 두 지점(백로그 승인, push 전 코드 리뷰)만 정직하게 사람에게 남겼다. 규칙을
벗어나는 순간 액션 자체가 실행되지 않는다는 게 핵심이다.

## 무엇을 만들었나

Claude Code의 [hooks](https://code.claude.com/docs/en/hooks) 이벤트마다 걸리는 28개
스크립트 — backlog.md 워크플로 전용 5개 + 프로젝트 종류와 무관하게 항상 켜져 있는 범용
...
- **기계적 강제 vs 사람 판단을 명확히 나눔** — 검증 가능한 건 훅으로 막고, 백로그 승인/
  push 전 코드 리뷰 두 곳만 사람에게 남긴다.
- **완전 독립형** — 훅끼리 서로 import하지 않는다. 코드 중복은 있지만 한 훅의 버그가
  다른 훅을 절대 안 깨뜨린다.
- **Python으로 작성** — 원래 여러 개는 bash 스크립트였다. Python으로 옮긴 건 셸보다
  가독성이 낫고, 작성자가 Python 생태계에 더 익숙하다는 실용적인 이유가 컸다 — 여러
  대안을 검토했지만 정답이 하나만 있는 문제는 아니었다.

## 훅 — Claude Code 생애주기별

Claude Code 훅은 특정 이벤트(생애주기 단계)에 등록되어 그 시점에만 실행된다. 이 저장소의
28개 훅이 어느 단계에 걸려있는지 전부 나열하면:
...
`instructions_audit.py`) 행마다 반복해서 나열했다 — 실제 `settings.hooks.json` 등록과
1:1로 대응시키기 위해서다.
...
- **전체 스펙**(Phase 명세, HIL 표, 설치/삭제, 파일 구성) → `backlog doc view doc-2`
