...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 hooks/dedup_drift_guard.py 신설: 스크립트 내부에 REGISTRY(함수명 -> 동일해야 하는 파일 목록) 명시, 최초 항목은 is_backlog_project -> [require_active_task.py, pre_commit_check.py, pre_push_check.py, block_stop_if_dirty.py, session_start.py]
- [ ] #2 matcher Bash, if Bash(git *) — git commit 실행 전에 REGISTRY의 각 함수 소스를 파일별로 추출해 비교, 하나라도 다르거나 누락되면 어느 파일이 다른지 구체적으로 보여주며 exit code 2로 커밋 차단
- [ ] #3 cwd/hooks/ 밑에 REGISTRY가 가리키는 파일들이 없으면(claude-rails 저장소 자신이 아니면) 즉시 통과 — self-guard
- [ ] #4 test_dedup_drift_guard.py 추가: 전부 동일할 때 통과 / 하나라도 다를 때 차단 / 함수가 일부 파일에서만 누락됐을 때도 차단, 세 케이스 커버
- [ ] #5 settings.hooks.json에 새 훅 등록 (pre_commit_check.py와 같은 이벤트/matcher/if 블록에 추가)
<!-- AC:END -->
