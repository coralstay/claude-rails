...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 high 레벨 이상에 rm -rf 대상이 ..로 상위 디렉토리를 벗어나거나 cwd 밖 절대경로를 가리키는 경우를 새 패턴으로 추가 (cwd 내부 rm -rf는 그대로 통과)
- [ ] #2 test_block_dangerous_commands.py에 새 패턴의 positive/negative 케이스 추가 (정상 rm -rf ./dist 회귀 테스트 포함)
<!-- AC:END -->
